...
//Viewing all posts
const Posts = ({ post, user, setPosts }) => {
    //Updating like
    const updateLike = () => {
        axios.patch('https://woof-book.vercel.app/posts/' + post._id, {
            likes: post.likes + 1
        })
    ...
    return (
        <div className="card my-3" key={post._id}>
            <div className="d-inline">
                <img src={`data:image/jpg;base64,${user[0].pProfile}`} className="postPic" alt="Card cap" />
                {user.length > 0 ?
                    <button className="btn"
                        onClick={() => { return Navigate(`/user/${user[0]._id}`) }}>{user[0].pName}</button> : null}
            </div>
            <div className="card-body">
                {post.type === "image" ? <img src={`data:image/jpg;base64,${post.data}`} className="postImage"
                    alt="post" /> :
                    post.type === 'link' ? <a href={post.data} target="_blank" rel="noopener noreferrer">Link</a> :
                        <p className="card-text">{post.data}</p>}
                <div className="d-inline">
                    <button className="btn" onClick={updateLike}>
                    ...
                    <button className="btn">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                            className="bi bi-box-arrow-down" viewBox="0 0 16 16">
